...
# Token
TOKEN1=`curl -XGET 'http://localhost:14126/api/token'| jq -r '.token'`

TOKEN2=`curl -XGET 'http://localhost:24126/api/token'| jq -r '.token'`

TOKEN3=`curl -XGET 'http://localhost:34126/api/token'| jq -r '.token'`

# Check serverid values for server1 and server2
SERVERID1=`curl -XGET 'http://localhost:14126/api/serverid' -H 'x-access-token: ${TOKEN1}' | jq -r '.serverid'`

SERVERID2=`curl -XGET 'http://localhost:24126/api/serverid' -H 'x-access-token: ${TOKEN2}' | jq -r '.serverid'`

SERVERID3=`curl -XGET 'http://localhost:34126/api/serverid' -H 'x-access-token: ${TOKEN3}' | jq -r '.serverid'`

# Create db1 on server1
curl -XPOST 'http://localhost:34126/api/db1' -H 'Content-Type: application/json' -H @- <<EOS 
